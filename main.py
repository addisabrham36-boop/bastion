...
def main():
    ...
    if args.upstream:
        print(f"  🎯  Default Upstream Target: {args.upstream}")
    else:
        print(f"  🎯  Domain Routing:          Managed via Dashboard (http://{args.host}:{args.dashboard_port})")
    print("━" * 68)
    print(f"  👉  Open http://{args.host}:{args.dashboard_port} to manage 100+ rules & add protected domains.")
    print(f"  👉  Point your DNS / traffic to http://{args.host}:{args.proxy_port} to protect your web servers.")
